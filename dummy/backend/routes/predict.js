...
// ──────────────────────────────────────────────
// POST /api/predict/symptoms — Symptom Checker
// ──────────────────────────────────────────────
router.post('/symptoms', async (req, res) => {
    try {
        ...
        if (!age || !symptoms || symptoms.length === 0) {
            return res.status(400).json({ error: "Age and at least one symptom are required" });
        }

        const mlResponse = await fetch("http://localhost:8000/predict", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ age: parseInt(age), symptoms })
        });
        ...
        // Compute a proper risk score
        let riskScore;
        if (mlData.predicted_condition === "Healthy") {
            riskScore = Math.round(100 - mlData.confidence);
        } else {
            riskScore = Math.round(mlData.confidence);
        }

        let riskLevel = "Low";
        if (riskScore >= 70) riskLevel = "High";
        else if (riskScore >= 40) riskLevel = "Moderate";
        else if (riskScore >= 20) riskLevel = "Mild";

        res.json({
            predicted_condition: mlData.predicted_condition,
            risk_level: riskLevel,
            risk_score: riskScore,
            confidence: mlData.confidence,
            disease_risks: mlData.disease_risks || []
        });
